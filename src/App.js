...
const App = () =>{
  ...
  const handleKeyPress= (key)=>{
    ...
    //allows user to remove previous input character
    if(key==="⌫"){
  ...
  //selects random word from data.js
  useEffect(() => {
  ...
  //checks if words and characters enter matches the answer
  const enterBoardWord =(word) =>{
    ...
    //for loop to check characters match answer
    ...
    if(matchCount === 5){
      ...
      status="LOST";
      handleMessage("Answer is " + boardData.solution + ". Good luck next time!")
    }
